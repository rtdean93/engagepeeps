...
class OrderForms extends OrderModel {
	 ...
	 *
	 * @return array An array of key/value pairs where each key is the template and each value is array of order types that are supported by the template
	 */
	public function getTemplates() {
		...
		while (($dir = readdir($dh)) !== false) {
			...
			$templates[$dir] = $this->getSupportedTypes($dir);
		}
		closedir($dh);
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array("company_id", "label", "name", "template", "type", "client_group_id",
