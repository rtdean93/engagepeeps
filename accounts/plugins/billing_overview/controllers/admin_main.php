...
class AdminMain extends BillingOverviewController {
	...
	public function update() {
		...
		$settings = array("revenue_today", "revenue_month", "revenue_year", "invoiced_today", "invoiced_month",
			"balance_outstanding", "balance_overdue", "scheduled_cancelation", "services_active", "services_added_today",
			"services_canceled_today", "graph_revenue", "graph_revenue_year", "graph_invoiced", "show_legend");
		...
		foreach ($settings as $setting) {
			...
			switch ($setting->key) {
				...
				case "graph_revenue_year":
					// Get the graph data
					$now = $this->Date->format("c");
					$start_date = $this->Date->format("Y-01-01 00:00:00", strtotime($now));
					$end_date = $this->Date->format("Y-12-31 23:59:59", strtotime($now));
	...
	private function getGraphBetween($key, $start_date, $end_date, $currency, $interval = "month") {
		...
		$start_day_format = ($month_interval && date("j", strtotime($start_date)) == "1" ? "01" : "d");
		
		while ((strtotime($end_date) > $end)) {
