...
class SupportManagerTickets extends SupportManagerModel {
	...
	public function close($ticket_id, $staff_id = null) {
		...
		$vars = array(
			'details' => Language::_("SupportManagerTickets.log.status", true, $statuses['closed']),
			'type' => "log"
		);
	...
	private function getReplies($ticket_id, array $types = null) {
		$fields = array("support_replies.*", 'IF(support_replies.staff_id IS NULL, contacts.first_name, staff.first_name)' => "first_name",
			'IF(support_replies.staff_id IS NULL, contacts.last_name, staff.last_name)' => "last_name",
			'IF(support_replies.staff_id IS NULL, contacts.email, staff.email)' => "email"
		);
		
		$this->Record->select($fields, false)->
			select(array('IF(staff.id IS NULL, IF(support_tickets.email IS NULL, ?, ?), ?)' => "reply_by"), false)->
	...
	private function getTickets($status = null, $staff_id = null, $client_id = null, $ticket_id = null) {
		...
		$fields = array("support_tickets.*", 'support_replies.date_added' => "last_reply_date",
			'support_departments.name' => "department_name", "support_departments.company_id");
		$last_reply_fields = array(
			'IF(support_replies.staff_id IS NULL, IF(support_tickets.email IS NULL, ?, ?), ?)' => "last_reply_by",
			'IF(support_replies.staff_id IS NULL, contacts.first_name, staff.first_name)' => "last_reply_first_name",
			'IF(support_replies.staff_id IS NULL, contacts.last_name, staff.last_name)' => "last_reply_last_name",
