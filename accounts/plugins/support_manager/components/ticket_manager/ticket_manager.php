...
class TicketManager {
	...
	public function processDepartmentEmails() {
		...
		Loader::load(PLUGINDIR . "support_manager" . DS . "vendors" . DS . "mime_mail_parser" . DS . "MimeMailParser.class.php");
		
		Loader::loadModels($this, array("SupportManager.SupportManagerDepartments"));
		
		// Download messages for reach department
		foreach ($this->SupportManagerDepartments->getByMethod($company_id, array('pop3', 'imap')) as $department) {
	...
	public function processTicketFromEmail(MimeMailParser $email, $department = null) {
		...
		if (!isset($this->EmailParser))
			Loader::loadComponents($this, array("SupportManager.EmailParser"));
			...
			if ($ticket_info['valid']) {
				...
				if ($ticket) {
					...
					if ($reply['staff_id'] == null && $reply['client_id'] == null &&
						($department = $this->SupportManagerDepartments->get($ticket->department_id)) && $department->clients_only) {
					...
					if (!$reply_id) {
						...
						if ($this->SupportManagerTickets->checkLoopBack($email, $this->max_reply_limit, $this->reply_period)) {
							...
							$additional_tags = array('SupportManager.ticket_updated' => array('update_ticket_url' => $this->Html->safe($hostname . $client_uri . "plugin/support_manager/client_tickets/reply/" . $ticket->id . "/")));
		...
		// Handle creating a new ticket
			...
			foreach ($to as $address) {
				...
				if ($department) {
					...
					if ($this->SupportManagerTickets->checkLoopBack($email, $this->max_reply_limit, $this->reply_period)) {
						// Send the email associated with this ticket
						$additional_tags = array('SupportManager.ticket_updated' => array('update_ticket_url' => $this->Html->safe($hostname . $client_uri . "plugin/support_manager/client_tickets/reply/" . $ticket_id . "/")));
						$this->SupportManagerTickets->sendEmail($reply_id, $additional_tags);
					}
					return;
				}
	...
	private function sendBounceNotice(MimeMailParser $email, $client_id = null) {
		...
		$this->Emails->send("SupportManager.ticket_bounce", Configure::get("Blesta.company_id"), $lang, $to, $tags, null, null, null, $options);
	}
