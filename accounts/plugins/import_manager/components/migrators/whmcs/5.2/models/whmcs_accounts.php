	...
	public function get($with_refund = false) {
		...
		if ($with_refund) {
			...
			*/
			...
			$subquery = $this->remote->select(array('SUM(ref.amountout)' => "refund", "ref.refundid"))->
				from(array('tblaccounts' => "ref"))->where("ref.refundid", ">", 0)->
				group(array("ref.refundid"))->get();

			$values = $this->remote->values;
			$this->remote->reset();
		}
		
		$fields = array("tblaccounts.*", 'trans_currencies.code' => "trans_currency", 'client_currencies.code' => "client_currency");
		if ($with_refund)
			$fields[] = "refunds.refund";
			
		$this->remote->select($fields)->from("tblaccounts")->
			leftJoin(array('tblcurrencies' => "trans_currencies"), "trans_currencies.id", "=", "tblaccounts.currency", false);
			
		if ($with_refund) {
			$this->remote->appendValues($values)->
			leftJoin(array($subquery => "refunds"), "refunds.refundid", "=", "tblaccounts.id", false);
		}
		
		$this->remote->leftJoin("tblclients", "tblclients.id", "=", "tblaccounts.userid", false)->
			leftJoin(array('tblcurrencies' => "client_currencies"), "client_currencies.id", "=", "tblclients.currency", false);
		
		return $this->remote->getStatement();
	}
	
	public function getOpenCredits() {
		$fields = array('tblclients.id' => "userid", "tblclients.credit", 'tblcurrencies.code' => "currency");
