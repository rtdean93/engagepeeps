...
class WhmcsMigrator extends Migrator {
	...
	protected function importClients() {
		...
		foreach ($clients as $client) {
			...
			$vars = array(
				...
				'first_name' => $this->decode($client->firstname),
				'last_name' => $this->decode($client->lastname),
	...
	protected function importServices() {
		...
		foreach ($services as $service) {
			...
			$vars = array(
				...
				'id_format' => "{num}",
				'id_value' => $service->id,
				'pricing_id' => $this->getPricingId($this->WhmcsServices->getTerm($service->billingcycle), $package, $currency, $service->amount),
		...
		foreach ($services as $service) {
			...
			$vars = array(
				...
				'id_format' => "{num}",
				'id_value' => $service->id,
				'pricing_id' => $this->getPricingId($this->WhmcsServices->getTerm($service->registrationperiod), $package, $currency, $service->recurringamount),
				'client_id' => $this->mappings['clients'][$service->userid],
				'module_row_id' => $module_row_id,
				'coupon_id' => null,
				'qty' => 1,
	...
	private function installModule($module, $mapping = null) {
		...
		$this->mappings['modules'][$module] = $module_id;
		return $module_id;
	}
	...
	/**
	 * Get the pricing ID for the given term, package, and currency
	 *
	 ...
	 */
	private function getPricingId($term, $package, $currency = null, $amount = null) {
		if (!is_array($term))
			return null;
		...
		if ($package) {
			foreach ($package->pricing as $price) {
				if ($price->term == $term['term'] && $price->period == $term['period']) {
					$pricing_id = $price->id;
					if ($price->currency == $currency && ($amount === null || (string)$amount == (string)$price->price)) {
						break;
					}
				}
			}
		...
		// If no pricing ID found, add default pricing
		if (!$pricing_id) {
			$pricing = array(
				array(
					'term' => $term['term'],
