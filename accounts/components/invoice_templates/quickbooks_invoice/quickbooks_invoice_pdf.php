...
class QuickbooksInvoicePdf extends TcpdfWrapper {
	...
	private function drawInvoiceInfo() {
		$data = array(
			array(
				...
				'invoice_id'=>Language::_("QuickbooksInvoice.invoice_id_code", true)
			),
			array(
				'client_id'=>$this->invoice->client->id_code,
				'date'=>date($this->invoice->client->settings['date_format'], strtotime($this->invoice->date_billed)),
	...
	private function drawInvoiceSubInfo() {
		$data = array(
			...
			array(
				'date_due'=>date($this->invoice->client->settings['date_format'], strtotime($this->invoice->date_due))
			)
		);
