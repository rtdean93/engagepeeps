...
class DefaultInvoicePdf extends TcpdfWrapper {
	...
	private function drawPaidWatermark() {
		// Show paid watermark
		if (!empty($this->meta['display_paid_watermark']) && $this->meta['display_paid_watermark'] && ($this->invoice->date_closed != null)) {
			...
			$data = array(
				array('col'=>Language::_("DefaultInvoice.watermark_paid", true))
			);
	...
	private function drawTallies() {
		...
		if (!empty($this->invoice->note_public)) {
			...
			$data = array(
				array(Language::_("DefaultInvoice.notes_heading", true)),
		...
		// Set subtitle
		$data = array(
			array('notes'=>null,'label'=>Language::_("DefaultInvoice.subtotal_heading", true),'price'=>$this->CurrencyFormat->format($this->invoice->subtotal, $this->invoice->currency, self::$standard_num_options))
		);
		// Set all taxes
		foreach ($this->invoice->taxes as $tax) {
			$data[] = array('notes'=>null,'label'=>Language::_("DefaultInvoice.tax_heading", true, $tax->name, $tax->amount),'price'=>$this->CurrencyFormat->format($tax->tax_total, $this->invoice->currency, self::$standard_num_options));
		}
		// Set total
		$data[] = array('notes'=>null,'label'=>Language::_("DefaultInvoice.total_heading", true),'price'=>$this->CurrencyFormat->format($this->invoice->total, $this->invoice->currency));
	...
	private function drawInvoiceType() {
		$data = array(
			array(Language::_("DefaultInvoice.type_" . $this->invoice->status, true))
		);
		$options = array(
	...
	private function drawPageNumber() {
		$data = array(
			array(Language::_("DefaultInvoice.page_of", true, $this->getGroupPageNo(), $this->getPageGroupAlias()))
		);
		$options = array(
			'font_size'=>self::$font_size_alt,
			'font_style' => "B",
