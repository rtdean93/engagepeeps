...
class GatewayPayments {
	...
	public function __construct() {
		...
		Loader::loadComponents($this, array("Gateways", "Input"));
		Loader::loadHelpers($this, array("DataStructure","CurrencyFormat"=>array($this->company_id)));
		Loader::loadModels($this, array("Accounts", "Clients", "Companies", "Contacts", "Emails", "GatewayManager", "Transactions", "Countries", "States"));
	}
	 ...
	 * @param int $gateway_id The ID of the nonmerchant gateway to fetch, if null will return all nonmerchant gateways for the currency
	 * @return array An array of HTML, indexed by the gateway name
	 */
	public function getBuildProcess(array $contact_info, $amount, $currency, array $invoice_amounts=null, array $options=null, $gateway_id=null) {
		
		$html = array();
		if ($gateway_id !== null)
			$gateways = array($this->GatewayManager->getInstalledNonmerchant($this->company_id, null, $gateway_id, $currency));
		else
		...
		foreach ($gateways as $gateway) {
			...
			$html[$gateway->class] = (array)$gateway_obj->buildProcess($contact_info, $amount, $invoice_amounts, $options);
		}
	 ...
	 * @return array An array of HTML, indexed by the gateway name
	 */
	public function getBuildAuthorize(array $contact_info, $amount, $currency, array $invoice_amounts=null, array $options=null, $gateway_id=null) {
		...
		foreach ($gateways as $gateway) {
			...
			$html[$gateway->class] = (array)$gateway_obj->buildAuthorize($contact_info, $amount, $invoice_amounts, $options);
		}
	 ...
	 * @param array An array of GET parameters passed into the callback request
	 * @param array An array of POST parameters passed into the callback request
	 * @return object An object representing the transaction if attempted, void otherwise. Check GatewayPayments::errors(), as some transactions may be attempted and yet still produce errors
	 */
	public function processNotification($gateway_name, array $get, array $post) {
		...
		if (is_array($response)) {
			...
			if ($trans_data['parent_transaction_id'])
				$parent_transaction = $this->Transactions->getByTransactionId($trans_data['parent_transaction_id'], $trans_data['client_id'], $gateway->id);
			
			// Fetch the existing transaction in the system for this gateway (if available)
			$transaction = null;
			if ($trans_data['transaction_id'])
				$transaction = $this->Transactions->getByTransactionId($trans_data['transaction_id'], null, $gateway->id);
	 ...
	 * @return object An object representing the transaction if attempted, void otherwise. Check GatewayPayments::errors(), as some transactions may be attempted and yet still produce errors
	 */
	public function processPayment($client_id, $type, $amount, $currency, array $account_info=null, $account_id=null, array $options=null) {
		...
		$account_info['state'] = (array)$this->States->get(isset($account_info['country']['alpha2']) ? $account_info['country']['alpha2'] : null, $account_info['state']);
		...
		if ($this->Input->validates($vars)) {
			switch ($type) {
				case "cc":
					if ($stored)
						$response = $gateway_obj->processStoredCc($account->client_reference_id, $account->reference_id, $amount, $apply_amounts);
					else
						$response = $gateway_obj->processCc($account_info, $amount, $apply_amounts);
					break;
				case "ach":
					if ($stored)
						$response = $gateway_obj->processStoredAch($account->client_reference_id, $account->reference_id, $amount, $apply_amounts);
					else
			...
			$transaction_id = $this->Transactions->add(array(
				'client_id' => $client_id,
				'amount' => $amount,
				'currency' => $currency,
				'type' => $type,
			...
			if (($errors = $this->Transactions->errors()))
				...
				if (!empty($apply_amounts) && $response['status'] == "approved")
					$this->Transactions->apply($transaction_id, array('amounts'=>$apply_amounts));
				...
				if ($options['email_receipt']) {
					...
					if ($type == "cc") {
						...
						$tags['card_type'] = isset($card_types[$account_info['type']]) ? $card_types[$account_info['type']] : null;
					}
					// Set bank account specific tags
					else {
						$account_types = $this->Accounts->getAchTypes();
						$tags['account_type'] = isset($account_types[$account_info['type']]) ? $account_types[$account_info['type']] : null;
					}
					
					$this->Emails->send("payment_" . $type . "_" . $response['status'], $this->company_id, $client->settings['language'], $email, $tags, null, null, null, array('to_client_id'=>$client->id, 'from_staff_id'=>$options['staff_id']));
				}
	 ...
	 * @param string $type The type of payment to submit "cc" or "ach", NOTE: Only "cc" is supported, but this varaible remains for consistency (and possible future support)
	 ...
	 * @return object An object representing the transaction if attempted, void otherwise. Check GatewayPayments::errors(), as some transactions may be attempted and yet still produce errors
	 */
	public function authorizePayment($client_id, $type, $amount, $currency, array $account_info=null, $account_id=null, array $options=null) {
		...
		if (empty($account_info)) {
			...
			if ($account->gateway_id)
				$stored = true; // must be processed using stored payment account methods
		}
		else {
			if ($type == "cc")
				$account_info['type'] = $this->Accounts->creditCardType($account_info['card_number']);
		...
		if ($this->Input->validates($vars)) {
			switch ($type) {
				case "cc":
					if ($stored)
						$response = $gateway_obj->authorizeStoredCc($account->client_reference_id, $account->reference_id, $amount, $apply_amounts);
					else
	...
	public function capturePayment($client_id, $transaction_id, $amount=null, array $options=null) {
		...
		$gateway = $this->GatewayManager->getInstalledMerchant($this->company_id, $transaction->currency, $transaction->gateway_id);
		...
		if ($this->Input->validates($vars)) {
			switch ($transaction->type) {
				case "cc":
					if ($stored)
						$response = $gateway_obj->captureStoredCc($account->client_reference_id, $account->reference_id, $transaction->reference_id, $transaction->transaction_id, $amount);
					else
						$response = $gateway_obj->captureCc($transaction->reference_id, $transaction->transaction_id, $amount);
					break;
			}
			...
			if (($errors = $this->Transactions->errors()))
				...
				// invoices
				if (!empty($apply_amounts) && $response['status'] == "approved")
					$this->Transactions->apply($transaction->id, array('amounts'=>$apply_amounts));
				...
				if ($options['email_receipt'] === true || $options['email_receipt'] == "true") {
					...
					$this->Emails->send("payment_" . $transaction->type . "_" . $response['status'], $this->company_id, $client->settings['language'], $email, $tags, null, null, null, array('to_client_id'=>$client->id, 'from_staff_id'=>$options['staff_id']));
				}
	 ...
	 * @return mixed An object representing the transaction if attempted, void otherwise. Check GatewayPayments::errors(), as some transactions may be attempted and yet still produce errors
	 */
	public function refundPayment($client_id, $transaction_id, $amount=null, array $options=null) {
		...
		if ($transaction->gateway_type == "nonmerchant")
			$gateway = $this->GatewayManager->getInstalledNonmerchant($this->company_id, null, $transaction->gateway_id);
		else
			$gateway = $this->GatewayManager->getInstalledMerchant($this->company_id, $transaction->currency, $transaction->gateway_id);
		...
		if ($this->Input->validates($vars)) {
			switch ($transaction->type) {
				case "cc":
					if ($stored)
						$response = $gateway_obj->refundStoredCc($account->client_reference_id, $account->reference_id, $transaction->reference_id, $transaction->transaction_id, $amount);
					else
						$response = $gateway_obj->refundCc($transaction->reference_id, $transaction->transaction_id, $amount);
					break;
				case "ach":
					if ($stored)
						$response = $gateway_obj->refundStoredAch($account->client_reference_id, $account->reference_id, $transaction->reference_id, $transaction->transaction_id, $amount);
					else
						$response = $gateway_obj->refundAch($transaction->reference_id, $transaction->transaction_id, $amount);
					break;
				default:
					// Handle non-merchant gateway refund
					if ($transaction->gateway_type == "nonmerchant")
						$response = $gateway_obj->refund($transaction->reference_id, $transaction->transaction_id, $amount, (isset($options['notes']) ? $options['notes'] : null));
					break;
			}
	...
	public function voidPayment($client_id, $transaction_id, array $options=null) {
		...
		if ($this->Input->validates($vars)) {
			switch ($transaction->type) {
				case "cc":
					if ($stored)
						$response = $gateway_obj->voidStoredCc($account->client_reference_id, $account->reference_id, $transaction->reference_id, $transaction->transaction_id);
					else
						$response = $gateway_obj->voidCc($transaction->reference_id, $transaction->transaction_id);
					break;
				case "ach":
					if ($stored)
						$response = $gateway_obj->voidStoredAch($account->client_reference_id, $account->reference_id, $transaction->reference_id, $transaction->transaction_id);
					else
	 ...
	 * @return mixed False if the gateway does not support off-site payment accounts or is not enabled for off-site payment accounts, void for all other errors (errors set, use GatewayPayments::errors() to fetch errors), else sets an array containing:
	...
	public function storeAccount($type, array $vars) {
		...
		$client_reference_id = $this->Accounts->getClientReferenceId($client->id, $gateway->id);
		
		$gateway_obj = $this->initGateway($gateway, $currency);
		
		// Verify gateway works off-site and is enabled to store account off-site
		if (!$this->implementsGateway($type, $gateway_obj, true) || (($type == "ach" && !$gateway_obj->requiresAchStorage()) || ($type == "cc" && !$gateway_obj->requiresCcStorage())))
			return false;
		...
		if (!($errors = $gateway_obj->errors())) {
			if (!$response)
				...
				return array_merge(array('gateway_id'=>$gateway->id), $response);
		}
		else
			$this->Input->setErrors($errors);
	}
	...
	public function updateAccount($type, $account, array $vars) {
		...
		// If the payment account is not set up under a gateway there's no point in attempting to update it because it won't exist
		...
		switch ($type) {
			case "cc":
				$response = $gateway_obj->updateCc($account_info, (array)$contact, $account->client_reference_id, $account->reference_id);
				break;
			case "ach":
				$response = $gateway_obj->updateAch($account_info, (array)$contact, $account->client_reference_id, $account->reference_id);
				break;
		}
	...
	public function removeAccount($type, $account) {
		...
		// If the payment account is not set up under a gateway there's no point in attempting to remove it because it won't exist
		if (!isset($account->gateway_id) || !$account->gateway_id)
			return false;
	 ...
	 * @param boolean $off_site True if attempting to verify support for off site payments, false otherwise
	 * @return boolean True if the gateway object supports the appropriate interface, false otherwise
	 */
	 ...
	 *
	 * @param array $invoices A key/value array where each key represents an invoice ID and the value represents the apply amount
	...
	private function invoiceArrayToAmounts(array $invoices=null) {
		...
		if (!empty($invoices)) {
			foreach ($invoices as $id => $amount)
				$amounts[] = array('invoice_id' => $id, 'amount' => $amount);
		}
		return $amounts;
	}
	 ...
	 * @param string $types A string containing the types of payments allowed (pipe delimited, e.g. "ach|cc" or just "cc")
	 * @param boolean $off_site True if attempting to verify support for off site payments, false otherwise
	 */
	private function processPaymentRules($gateway_obj, $types="ach|cc", $off_site) {
		$rules = array(
			'type' => array(
				'valid' => array(
					'rule' => array("matches", "/^(" . $types . ")$/i"),
		...
		if ($types != "other") {
			$rules['type']['instance'] = array(
				'rule' => array(array($this, "implementsGateway"), $gateway_obj, $off_site),
