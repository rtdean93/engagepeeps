 ...
 *
 * Documentation on the AIM (Advanced Integration Method) API can be found at: http://www.authorize.net/support/AIM_guide.pdf
 ...
 * eCheck payment processing utilizes the AIM API, but has supplemental documentation at: http://developer.authorize.net/guides/echeck.pdf
 *
...
class AuthorizeNet extends MerchantGateway implements MerchantAch, MerchantAchOffsite, MerchantCc, MerchantCcOffsite {
	...
	public function encryptableFields() {
		return array("login_id", "transaction_key");
	}
	...
	public function getName() {
		return Language::_("Authorize_net.name", true);
	}
	...
	/**
	 * Informs the system of whether or not this gateway is configured for offsite customer information storage for ACH payments
	 *
	...
	/**
	 * Informs the system of whether or not this gateway is configured for offsite customer information storage for credit card payments
	 *
	 ...
	 *
	 * @return array A numerically indexed array containing all currency codes this gateway supports
	 */
	public function getCurrencies() {
		return array("CAD", "GBP", "EUR", "USD");
	}
	...
	public function storeCc(array $card_info, array $contact, $client_reference_id=null) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->store("cc", $card_info, array('customer_id'=>$contact['client_id']), $client_reference_id);
	...
	public function updateCc(array $card_info, array $contact, $client_reference_id, $account_reference_id) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->update("cc", $card_info, array('customer_id'=>$contact['client_id']), $client_reference_id, $account_reference_id);
		$this->logCim($response);
	...
	public function processStoredCc($client_reference_id, $account_reference_id, $amount, array $invoice_amounts=null) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->authCapture($client_reference_id, $account_reference_id, $amount, $invoice_amounts);
		$this->logCim($response);
	...
	public function authorizeStoredCc($client_reference_id, $account_reference_id, $amount, array $invoice_amounts=null) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->auth($client_reference_id, $account_reference_id, $amount, $invoice_amounts);
		$this->logCim($response);
	...
	public function captureStoredCc($client_reference_id, $account_reference_id, $transaction_reference_id, $transaction_id, $amount, array $invoice_amounts=null) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->capture($client_reference_id, $account_reference_id, $transaction_id, $amount, $invoice_amounts);
		$this->logCim($response);
	...
	public function authorizeCc(array $card_info, $amount, array $invoice_amounts=null) {
		$this->loadApi("AIM");
		$response = $this->AuthorizeNetAim->authCc($card_info, $amount, $invoice_amounts);
		$this->logAim($response);
	...
	public function captureCc($reference_id, $transaction_id, $amount, array $invoice_amounts=null) {
		$this->loadApi("AIM");
		$response = $this->AuthorizeNetAim->captureCc($transaction_id, $amount, $invoice_amounts);
		$this->logAim($response);
	...
	public function storeAch(array $account_info, array $contact, $client_reference_id=null) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->store("ach", $account_info, array('customer_id'=>$contact['client_id']), $client_reference_id);
		$this->logCim($response);
	...
	public function updateAch(array $account_info, array $contact, $client_reference_id, $account_reference_id) {
		$this->loadApi("CIM");
		$response = $this->AuthorizeNetCim->update("ach", $account_info, array('customer_id'=>$contact['client_id']), $client_reference_id, $account_reference_id);
		$this->logCim($response);
	...
	public function processAch(array $account_info, $amount, array $invoice_amounts=null) {
		...
		return array(
			'status'=>$status,
			'reference_id'=>substr($this->ifSet($account_info['account_number']), -4) . ":" . substr($this->ifSet($account_info['routing_number']), -4),
			'transaction_id'=>$this->ifSet($response['x_trans_id']),
			'message'=>$this->ifSet($response['x_response_reason_text'])
		);
	}
	...
	public function refundAch($reference_id, $transaction_id, $amount) {
		...
		$account_last4 = $this->ifSet($temp[0]);
		$routing_last4 = $this->ifSet($temp[1]);
		unset($temp);
	...
	private function setTransactionErrors($response) {
		if (isset($response['x_response_code']) && ($response['x_response_code'] != "2" || $response['x_response_code'] == "3")) {
		...
		switch ($type) {
			case "aim":
				if (!isset($this->AuthorizeNetAim)) {
					...
					$this->AuthorizeNetAim = new AuthorizeNetAim($this->meta['login_id'], $this->meta['transaction_key'], $this->meta['test_mode'] == "true", $this->meta['dev_mode'] == "true");
				}
			...
			case "cim":
				if (!isset($this->AuthorizeNetCim)) {
					...
					$this->AuthorizeNetCim = new AuthorizeNetCim($this->meta['login_id'], $this->meta['transaction_key'], $this->meta['dev_mode'] == "true", $this->meta['validation_mode']);
				}
	...
	private function logAim($response) {
		...
		$this->log($this->AuthorizeNetAim->getUrl(),  serialize($this->maskData($this->AuthorizeNetAim->getParams(), $mask_fields)), "input", true);
		
		// Log response from the gateway (which is successful if, and only if, we received a response from the gateway)
		$this->log($this->AuthorizeNetAim->getUrl(), $this->AuthorizeNetAim->getRawResponse(), "output", isset($response['x_response_code']));
	}
	
	/**
	 * Log the request performed using the CIM API
	 *
	 * @param boolean Whether or not the response was successful
	 */
