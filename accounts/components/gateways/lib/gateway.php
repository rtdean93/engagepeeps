...
abstract class Gateway {
	 ...
	 * @param array $mask_fields An array of key/value pairs where each key identifies a key in $data and whose value is an array containing:
	 ...
	 * @param int $unmask_length The length and direction of characters to remain unmasked if not specified in $mask_fields array
	...
	protected function maskDataRecursive(array $data, array $mask_fields, $mask_char="x", $unmask_length=0) {
		foreach ($data as $key => $value) {
			if (is_array($value)) {
				...
				// Rule may be defined numerically, so search for a match on the field name within the list of mask fields for the rule
	 ...
	 * @param mixed $rule An array of rule setting, or a string to use $mask_char and $unmask_length default values instead. Values include:
	 * 	- char The character to use as the mask
	 * 	- length The length of the original data to remain unmasked. A negative number will leave that many characters unmasked from the end of the string, while a positive number will leave that many characters unmasked from the beginning of the string, 0 will mask all characters
