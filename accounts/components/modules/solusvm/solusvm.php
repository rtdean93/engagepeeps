...
class Solusvm extends Module {
	...
	public function getName() {
		return Language::_("Solusvm.name", true);
	}
	...
	public function moduleRowName() {
		return Language::_("Solusvm.module_row", true);
	}
	...
	public function moduleRowNamePlural() {
		return Language::_("Solusvm.module_row_plural", true);
	}
	...
	public function moduleGroupName() {
		return Language::_("Solusvm.module_group", true);
	}
	...
	public function validateService($package, array $vars=null, $edit=false) {
		...
		if ($edit) {
			$rules['solusvm_hostname']['format']['if_set'] = true;
			if (isset($rules['solusvm_template']))
				$rules['solusvm_template']['valid']['if_set'] = true;
		}
	...
	public function addService($package, array $vars=null, $parent_package=null, $parent_service=null, $status="pending") {
		...
		return array(
			array(
				...
				'value' => isset($response->vserverid) ? $response->vserverid : null,
				'encrypted' => 0
			),
			array(
	...
	public function editService($package, $service, array $vars=array(), $parent_package=null, $parent_service=null) {
		...
		if ($vars['use_module'] == "true") {
			...
			if (isset($delta['solusvm_template'])) {
				$data = array('template' => $delta['solusvm_template']);
				if (!$this->performAction("rebuild", $service_fields->solusvm_vserver_id, $row, $data))
					$this->Input->setErrors(array('api' => array('internal' => Language::_("Solusvm.!error.api.internal", true))));
				else
					$service_fields->solusvm_template = $delta['solusvm_template'];
			}
			
			// Update hostname (if changed)
			if (isset($delta['solusvm_hostname'])) {
				$data = array('hostname' => $delta['solusvm_hostname']);
			...
			if (isset($delta['solusvm_root_password'])) {
				$data = array('rootpassword' => $delta['solusvm_root_password']);
		...
		$fields = array();
		$encrypted_fields = array("solusvm_console_password", "solusvm_vnc_password", "solusvm_root_password", "solusvm_password");
	...
	public function cancelService($package, $service, $parent_package=null, $parent_service=null) {
		...
		if (($row = $this->getModuleRow())) {
			...
			try {
				// Load up the Virtual Server API
				$vserver_api = new SolusvmVserver($api);
				$params = array('vserverid' => $service_fields->solusvm_vserver_id, 'deleteclient' => "false");
	...
	public function suspendService($package, $service, $parent_package=null, $parent_service=null) {
		...
		if (($row = $this->getModuleRow())) {
			...
			try {
				...
				$params = array('vserverid' => $service_fields->solusvm_vserver_id);
	...
	public function addModuleRow(array &$vars) {
		$meta_fields = array("server_name", "user_id", "key", "host", "port");
		$encrypted_fields = array("user_id", "key");
	...
	public function getGroupOrderOptions() {
		return array('first'=>Language::_("Solusvm.order_options.first", true));
	}
	...
	public function getPackageFields($vars=null) {
		...
		$types = array('' => Language::_("Solusvm.please_select", true)) + $this->getTypes();
		$type = $fields->label(Language::_("Solusvm.package_fields.type", true), "solusvm_type");
		$type->attach($fields->fieldSelect("meta[type]", $types,
			$this->Html->ifSet($vars->meta['type']), array('id' => "solusvm_type")));
		$fields->setField($type);
		unset($type);
		
		// Set field whether client or admin may choose template
		$set_template = $fields->label(Language::_("Solusvm.package_fields.set_template", true), "solusvm_client_set_template");
		$admin_set_template = $fields->label(Language::_("Solusvm.package_fields.admin_set_template", true), "solusvm_admin_set_template");
		$client_set_template = $fields->label(Language::_("Solusvm.package_fields.client_set_template", true), "solusvm_client_set_template");
		$set_template->attach($fields->fieldRadio("meta[set_template]", "client",
			$this->Html->ifSet($vars->meta['set_template'], "client") == "client", array('id' => "solusvm_client_set_template", 'class' => "solusvm_chosen_template"), $client_set_template));
		$set_template->attach($fields->fieldRadio("meta[set_template]", "admin",
			$this->Html->ifSet($vars->meta['set_template']) == "admin", array('id' => "solusvm_admin_set_template", 'class' => "solusvm_chosen_template"), $admin_set_template));
		...
		$template->attach($fields->fieldSelect("meta[template]", $templates,
			$this->Html->ifSet($vars->meta['template']), array('id'=>"solusvm_template")));
		...
		$plan->attach($fields->fieldSelect("meta[plan]", $plans,
			$this->Html->ifSet($vars->meta['plan']), array('id'=>"solusvm_plan")));
		...
		$set_node->attach($fields->fieldRadio("meta[set_node]", "0",
			$this->Html->ifSet($vars->meta['set_node'], "0") == "0", array('id' => "set_node_group", 'class' => "solusvm_set_node"), $node_group));
		$set_node->attach($fields->fieldRadio("meta[set_node]", "1",
			$this->Html->ifSet($vars->meta['set_node']) == "1", array('id' => "set_node", 'class' => "solusvm_set_node"), $node));
		$fields->setField($set_node);
		
		// Set field for node groups
		$groups = array('' => Language::_("Solusvm.please_select", true)) + $node_groups;
		$node_group = $fields->label(Language::_("Solusvm.package_fields.node_group", true), "node_group");
		$node_group->attach($fields->fieldSelect("meta[node_group]", $groups,
			$this->Html->ifSet($vars->meta['node_group']), array('id' => "solusvm_node_group")));
	...
	public function getEmailTags() {
		return array(
			...
			'package' => array(),
			'service' => array("solusvm_vserver_id", "solusvm_console_user", "solusvm_console_password",
				"solusvm_hostname", "solusvm_main_ip_address", "solusvm_internal_ip", "solusvm_extra_ip_addresses",
				"solusvm_node", "solusvm_username", "solusvm_password", "solusvm_plan", "solusvm_root_password",
				"solusvm_template", "solusvm_type", "solusvm_virt_id", "solusvm_vnc_ip", "solusvm_vnc_port",
				"solusvm_vnc_password"
			)
		);
	}
	...
	public function getAdminAddFields($package, $vars=null) {
		...
		$host_name->attach($fields->fieldText("solusvm_hostname", $this->Html->ifSet($vars->solusvm_hostname), array('id'=>"solusvm_hostname")));
		...
		if (isset($package->meta->set_template) && isset($package->meta->type) &&
			...
			$template->attach($fields->fieldSelect("solusvm_template", $templates, $this->Html->ifSet($vars->solusvm_template), array('id'=>"solusvm_template")));
			// Set the label as a field
			$fields->setField($template);
		}
	...
	public function getClientTabs($package) {
		return array(
			'tabClientActions' => Language::_("Solusvm.tab_actions", true),
			'tabClientStats' => Language::_("Solusvm.tab_stats", true),
			'tabClientConsole' => Language::_("Solusvm.tab_console", true),
		);
	}
	...
	public function tabActions($package, $service, array $get=null, array $post=null, array $files=null) {
		...
		// Set default vars
		if (empty($vars))
			$vars = array('template' => $service_fields->solusvm_template, 'hostname' => $service_fields->solusvm_hostname);
				...
				case "password":
					...
					if (!empty($post)) {
						...
						if ($this->Input->validates($post)) {
							...
							$this->Services->edit($service->id, array('solusvm_root_password' => $post['password']));
				...
				case "hostname":
					...
					if (!empty($post)) {
						...
						if ($this->Input->validates($post)) {
							// Update the service hostname
							Loader::loadModels($this, array("Services"));
							$this->Services->edit($service->id, array('solusvm_hostname' => $post['hostname']));
				...
				case "reinstall":
					...
					if (!empty($post)) {
						...
						if ($this->Input->validates($post)) {
							...
							Loader::loadModels($this, array("Services"));
							$this->Services->edit($service->id, array('solusvm_template' => $post['template']));
	...
	private function consoleTab($package, $service, $client=false) {
		...
		if (in_array(strtolower($service_fields->solusvm_type), array("xen", "openvz"))) {
			...
			$type = "vnc";
			$session = array(
				'vnc_ip' => $service_fields->solusvm_vnc_ip,
				'vnc_password' => $service_fields->solusvm_vnc_password,
				'vnc_port' => $service_fields->solusvm_vnc_port
			);
		...
		$this->view->set("node_statistics", $this->getNodeStatistics($service_fields->solusvm_node, $module_row));
	...
	private function setConsoleSession($service, $module_row, $length=1) {
		...
		try {
			$server_api = new SolusvmVserver($api);
			$params = array('vserverid' => $service_fields->solusvm_vserver_id, 'access' => "enable", 'time' => (int)$length);
		...
		if ($response) {
			...
			foreach ($fields as $field => $name) {
				...
				$response->{$field} = array(
					...
					'percent_used_' . $name . "_formatted" => Language::_("Solusvm.!percent.used", true, (isset($values[3]) ? $values[3] : "")),
				);
			}
		}
	...
	private function getFieldsFromInput(array $vars, $package) {
		...
		$template = $package->meta->set_template == "admin" ? $package->meta->template : (isset($vars['solusvm_template']) ? $vars['solusvm_template'] : null);
		
		$fields = array(
			'type' => $package->meta->type,
			'hostname' => isset($vars['solusvm_hostname']) ? $vars['solusvm_hostname'] : null,
	...
	private function chooseNode($nodes, $module_row) {
		...
		if (count($nodes) == 1)
			...
			foreach ($nodes as $node_id) {
				...
				$disk = (property_exists($node_stats, "freedisk") && !empty($node_stats->freedisk) ? (float)$node_stats->freedisk : 0);
				$memory = (property_exists($node_stats, "freememory") && !empty($node_stats->freememory) ? (float)$node_stats->freememory : 0);
	 ...
	 * @return array An key/value array including the client's username and password. If the client already exists in SolusVM, then the password returned is null
	 */
	private function createClient($client_id, $username, $module_row) {
		...
		if ($response && $response->status != "success") {
			...
			if (!$response || $response->status != "success")
				$this->Input->setErrors(array('create_client' => array('failed' => Language::_("Solusvm.!error.create_client.failed", true))));
		}
