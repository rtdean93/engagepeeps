...
class Vpsdotnet extends Module {
	...
	public function getAdminAddFields($package, $vars=null, $client=false) {
		...
		// Allow AJAX requests to fetch/update service info fields
		$this->setAjaxFields($module_row, $package, $vars, $choose_fields);
		$please_wait = Language::_("Vpsdotnet.please_wait", true);
		...
		$fields->setHtml("
			<script type=\"text/javascript\">
				$(document).ready(function() {
					$('#vpsdotnet_cloud, #vpsdotnet_template_group').live('change', function() {
						var options_section = $(this).parents('ul').parent();
						var script = $(options_section).find('script');
						var form = $(this).closest('form');
						var submit_text = " . ($client ? "$(form).find('.button_row input').val();" : "$(form).find('.button_row a').text();") . "
						var url = $(form).attr('action');
						
						$(this).blestaRequest('post', url, $(form).serialize(), function(data) {
								if (data) {
									$(options_section).find('ul').remove();
									$(options_section).prepend(data);
									" . ($client ? "$(form).find('.button_row input[type=submit]').show();$(form).find('.button_row input[type=button]').remove();" : "$(form).find('.button_row a').addClass('submit').text(submit_text);") . "
								}
							},
							null,
							{dataType: 'json'}
						...
						$(this).parents('ul').find('input,select').attr('disabled', 'disabled');
						" . ($client ? "$(form).find('.button_row input').hide();$(form).find('.button_row input').before('<input type=\'button\' value=\'" . $please_wait . "\' class=\'submit-btn\' />');" : "$(form).find('.button_row a').removeClass('submit').text('" . $please_wait . "');") . "
					});
				});
			</script>
		");
		...
		$host_name->attach($fields->fieldText("vpsdotnet_hostname", $this->Html->ifSet($vars->vpsdotnet_hostname), array('id'=>"vpsdotnet_hostname")));
	...
	/**
	 * AJAX Sets service fields via AJAX. Outputs data via JSON to browser if this is an AJAX request.
	 * @see Vpsdotnet::getAdminAddFields()
	 *
	 * @param stdClass $module_row A stdClass object representing a single server
	 * @param stdClass $package A stdClass object representing the selected package
	 * @param mixed $vars A stdClass object representing a set of post fields
	 * @param array $fields A list of package meta fields that may be set by the client 
	 */
	private function setAjaxFields($module_row, $package, $vars=null, array $fields=array()) {
		#
		# TODO: this AJAX work-around should be removed when AJAX support is available for service fields
		#
		// Allow AJAX requests to fetch template info
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest") {
			$data = "";
			
			// Determine whether the cloud was given and can be set
			$cloud = $this->Html->ifSet($package->meta->cloud);
			$template_group = $this->Html->ifSet($package->meta->template_group);
			if (in_array("cloud", $fields) && isset($vars->vpsdotnet_cloud))
				$cloud = (int)$vars->vpsdotnet_cloud;
			if (in_array("template_group", $fields) && isset($vars->vpsdotnet_template_group))
				$template_group = $vars->vpsdotnet_template_group;
			
			if (!empty($cloud)) {
				$view = new View("service_fields", "default");
				$view->base_uri = $this->base_uri;
				$view->setDefaultView("components" . DS . "modules" . DS . "vpsdotnet" . DS);
				$please_select = array('' => Language::_("AppController.select.please", true));
				
				if (in_array("cloud", $fields))
					$view->set("clouds", $please_select + $this->getClouds($module_row));
				
				// Load the helpers required for this view
				Loader::loadHelpers($view, array("Form", "Html"));
				
				// Retrieve the template groups
				if (in_array("template_group", $fields) && !empty($cloud))
					$view->set("template_groups", $please_select + $this->getTemplateGroups($module_row, $cloud));
				if (in_array("template", $fields) && !empty($template_group))
					$view->set("templates", $please_select + $this->getTemplates($module_row, $cloud, $template_group));
