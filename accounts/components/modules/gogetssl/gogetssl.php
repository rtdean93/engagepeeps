...
class Gogetssl extends Module {
	...
	private function getApproverEmails($api, $package, $domain) {
		...
		if($this->isComodoCert($api, $package))
			$gogetssl_approver_emails = $gogetssl_approver_emails['ComodoApprovalEmails'];
		else
			$gogetssl_approver_emails = $gogetssl_approver_emails['GeotrustApprovalEmails'];
		return array_values($gogetssl_approver_emails);
	}
	...
	private function makeAddFields($package, $vars) {
		...
		if(!empty($_REQUEST['get_approver_emails'])) {
			die(json_encode($this->getApproverEmails($api, $package, $_REQUEST['get_approver_emails'])));
		}
		...
		$fields->setHtml("
			<script type=\"text/javascript\">
				...
				window.setTimeout('getApproverEmails();', 100);
				
				var PLEASE_SELECT = '".Language::_("GoGetSSL.please_select", true)."';
				
				function addToQuery(str) {
					var loc = document.location.href;
					var locQuery = loc.indexOf('?');
					if(locQuery <= 0)
						return '?' + str;
					return loc.substr(locQuery) + '&' + str;
				}
				
				function getApproverEmails() {
					$.getJSON(addToQuery('get_approver_emails=' + encodeURIComponent($('#gogetssl_fqdn').val())), function(data) {
						var emailField = document.getElementById('gogetssl_approver_email');
						emailField.innerHTML = '';
						var o = document.createElement('option');
						o.appendChild(document.createTextNode(PLEASE_SELECT));
						o.value = '';
						emailField.appendChild(o);
