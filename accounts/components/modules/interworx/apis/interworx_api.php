...
class InterworxApi {
	 ...
	 */
	private function apiQuery($api_controller = "", $action = "", $input = array()) {
	 ...
	 * 	- log data to be stored in a log.
	 */
	private function sendQuery($api_controller = "", $action = "", $input = array()) {
		...
		*/
		...
		$log = array();
		$log['parameters'] = array("apikey"    => $this->apikey,
		...
		if(isset($response["status"]) && $response["status"]) {
			return $this->getResponse(null, "reported_error ".$response["payload"], $log);
		}
	...
	private function getResponse($response, $error = "", $log = "") {
		...
		if(!isset($response["payload"]) || !isset($response["status"]) || !is_array($response)) {
			$response = array("status" => 0, "payload" => $response);
		}
				
		$results = new stdClass;
		$results->status = ( !$error && isset($response["status"]) && !$response["status"] && isset($response["payload"]) ? "success" : "error");
		...
		else if(!empty($response["payload"]) && $results->status == "error") {
	...
	public function createAccount($values) {
		...
		if ( !isset($values["username"]) || !isset($values["password"]) || !isset($values["domain"]) || !isset($values["email"])) {
		...
		if ( preg_match("/[^a-z0-9]/", $values["username"])) {
		...
		// Set Permissions
		$input["perms"] = array("SWACCOUNTS");		
		
		// Add Package information
		$packages = $this->listPackages();
		$package_details = false;
		
		if(!empty($values["plan"]) && isset($packages->status) && $packages->status == "success" && !empty($packages->response)) {
	 ...
	 * @link http://docs.interworx.com/interworx/api/index-Controller---nodeworx-siteworx.php
	 */
	public function modifyAccount($input = array()) {
	...
	public function listAccounts() {
		$input = array("perms" => array("SWACCOUNTS" ));
	...
	public function listPackages() {
		$input = array("perms" => array("RESELLER" ));
	...
	public function listUsage($domain = "") {
		...
		if(!$domain) {
			return $this->apiQuery("/nodeworx/siteworx", "listBandwidthAndStorage", array("perms" => array("SWACCOUNTS")));
		}
		
		$accounts = $this->apiQuery("/nodeworx/siteworx", "listBandwidthAndStorage", array("perms" => array("SWACCOUNTS")));
	...
	public function listIps() {
		return $this->apiQuery("/nodeworx/siteworx", "listFreeIps", array("perms" => array("SWACCOUNTS")));
	}
	...
	public function canConnect() {
		...
		$results = $this->apiQuery("/nodeworx/users", "listWorkingUser", array("perms" => array("NODEWORXUSER")));
