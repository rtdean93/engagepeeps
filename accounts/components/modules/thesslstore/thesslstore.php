...
class Thesslstore extends Module {
	...
	public function getApproverEmails($api, $package, $domain) {
		...
		return array_values($thesslstore_approver_emails->ApproverEmailList);
	}
	...
	private function makeAddFields($package, $vars) {
		...
		if(!empty($_GET['get_approver_emails'])) {
			die(json_encode($this->getApproverEmails($api, $package, $_GET['get_approver_emails'])));
		}
		...
		$fields->setHtml("
			<script type=\"text/javascript\">
				...
				var PLEASE_SELECT = '".Language::_("TheSSLStore.please_select", true)."';
				...
				function getApproverEmails() {
					$.getJSON(addToQuery('get_approver_emails=' + encodeURIComponent($('#thesslstore_fqdn').val())), function(data) {
						var emailField = document.getElementById('thesslstore_approver_email');
	...
	private function tabReissueInternal($package, $service, array $get=null, array $post=null, array $files=null) {
		...
		if(isset($post["thesslstore_csr"])) {
			...
			$vars = array(
				...
				'thesslstore_approver_email' => $post['thesslstore_approver_email'],
