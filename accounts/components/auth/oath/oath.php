...
class Oath {
	 ...
	 */
	private static $digits_power = array(1,10,100,1000,10000,100000,1000000,10000000,100000000);
	/**
	 ...
	 * @return boolean True if this OTP is valid, false otherwise
	 */
	public function checkHotp($otp, $counter, $vlav=0) {
		...
		for ($i=0; $i<$vlav; $i++) {
			if ($otp == self::hotp($secret, ($counter+$i), strlen($otp), $this->crypto))
				return true;
		}
		return false;
	}
	...
	public function checkTotp($otp, $time) {
		...
		for ($i=-$k; $i<$k; $i++) {
			if ($otp == self::totp($secret, ($time+($this->moving_factor*$i))/$this->moving_factor, strlen($otp), $this->crypto))
				return true;
		}
		return false;
	}
