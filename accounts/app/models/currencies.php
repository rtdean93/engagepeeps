...
class Currencies extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("currencies"));
	}
	 ...
	 *
	 * @return array An array of currency formats in key/value pairs where the key is the format and the value is an example
	 */
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			// Add a currency
			$fields = array("code", "company_id", "format", "prefix", "suffix",
				"exchange_rate", "exchange_updated"
			);
	...
	public function edit($currency_code, $company_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			// Update a currency
			$fields = array("format", "prefix", "suffix", "exchange_rate", "exchange_updated");
			$this->Record->where("company_id", "=", $company_id)->
				where("code", "=", $currency_code)->update("currencies", $vars);
		}
	}
	...
	public function delete($currency_code, $company_id) {
		$rules = array(
			'currency_code' => array(
				'in_use' => array(
					'rule' => array(array($this, "validateCurrencyInUse"), $company_id),
					'negate' => true,
					'message' => $this->_("Currencies.!error.currency_code.in_use", $currency_code)
				),
				'default' => array(
					'rule' => array(array($this, "validateCurrencyIsDefault"), $company_id),
					'negate' => true,
		...
		$vars = array('currency_code' => $currency_code);		
	 ...
	 * @return double The $amount converted between currencies
	 */
	public function convert($amount, $from_currency, $to_currency, $company_id) {
		...
		if (($from = $this->get($from_currency, $company_id)) && (($to = $this->get($to_currency, $company_id)))) {
			...
			$from_amount = $this->toDecimal($amount, $from->code, $company_id);
			$to_amount = ($to->exchange_rate == 0) ? 0 : ($from_amount*($to->exchange_rate/$from->exchange_rate));
			return $this->toDecimal($to_amount, $to->code, $company_id);
		}
		return $amount;
	}
	 ...
	 */
	public function toDecimal($value, $currency, $company_id=null, $decimals=2) {
		...
		if (is_float($value) || (is_numeric($value) && strpos($value, ".") !== false))
			return $this->truncateDecimal($value, 2, '.');
		...
		if ($formats[$currency][$company_id]) {
			...
			//$value = rtrim(ltrim($value, $formats[$currency][$company_id]->prefix), $formats[$currency][$company_id]->suffix);
			$value = str_replace(array($formats[$currency][$company_id]->prefix, $formats[$currency][$company_id]->suffix), "", $value);
		...
		return $this->truncateDecimal(number_format((float)$value, $decimals, '.', ''), 2, '.');
	}
	 ...
	 * @param int $company_id The company ID to fetch the currency for
	 * @param boolean $prefix True to include the prefix symbol for this currency, false otherwise
	 * @param boolean $suffix True to include the suffix symbol for this currency, false otherwise
	 * @param boolean $code True to include the currency code for this currency, false otherwise
	 * @param boolean $with_separator True to include the separator for this currency, false otherwise
	 * @param int $decimals The number of decimal places (precision)
	 * @return string The value formatted for the specified currency
	 * @see Currencies::toDecimal()
	 * @see Currencies::toCurrencyValue()
	 */
	public function toCurrency($value, $currency, $company_id=null, $prefix=true, $suffix=true, $code=false, $with_separator=true, $decimals=2) {
		if ($company_id === null)
			$company_id = Configure::get("Blesta.company_id");
		...
		switch ($formats[$currency][$company_id]->format) {
			...
			case "#,###.##":
				$value = $this->truncateDecimal(number_format($value, $decimals, '.', $with_separator ? ',' : ''), 2, '.');
				break;
			case "#.###,##":
				$value = $this->truncateDecimal(number_format($value, $decimals, ',', $with_separator ? '.' : ''), 2, ',');
				break;
			...
			case "#,##,###.##":
				...
				$value = $this->truncateDecimal($value, 2, '.');
				break;
			// Values with no decimal separator (unless explicitly given)
			case "# ###":
				$value = $this->truncateDecimal(number_format($value, $decimals, '.', $with_separator ? ' ' : ''), 0, '.');
				break;
			case "#.###":
				$value = $this->truncateDecimal(number_format($value, $decimals, ',', $with_separator ? '.' : ''), 0, ',');
				break;
	 ...
	 * @see Currencies::toCurrency()
	 */
	public function toCurrencyValue($value, $currency, $with_separator=true, $company_id=null, $decimals=2) {
	...
	private function getRules($vars) {
		$rules = array(
			'code' => array(
				...
				'exists' => array(
					'rule' => array(array($this, "validateCurrencyExists"), $this->ifSet($vars['company_id'])),
					'negate' => true,
					'message' => $this->_("Currencies.!error.code.exists", $this->ifSet($vars['code']), $this->ifSet($vars['company_id']))
				)
			),
			'company_id' => array(
				'exists' => array(
			...
			'format' => array(
				'format' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateCurrencyFormat")),
	 ...
	 * @return boolean True if the currency code is in use, false otherwise
	 */
	 ...
	 * @return boolean True if the currency is the default currency for this company, false otherwise
	 */
	public function validateCurrencyIsDefault($currency_code, $company_id) {
		...
		return (isset($default_currency['value']) && preg_match("/" . $default_currency['value'] . "/i", $currency_code)) ? true : false;
	}
	...
	public function updateRates() {
		...
		if (!isset($this->ExchangeRates))
			Loader::loadComponents($this, array("ExchangeRates"));
		if (!isset($this->Net))
		...
		$processor = $this->ifSet($company_settings['exchange_rates_processor']);
		$rate_processor = null;
		
		if (!empty($processor)) {
			// Attempt to create the processor
			try {
				$rate_processor = $this->ExchangeRates->create($processor, array($this->Net->create("Http")));
			}
			catch(Exception $e) {
				// Error, invalid processor
				$this->Input->setErrors(array('processor'=>array('invalid'=>$this->_("Currencies.!error.processor.invalid", true))));
			}
		}
		else {
			// Error, no processor is set
			$this->Input->setErrors(array('processor'=>array('empty'=>$this->_("Currencies.!error.processor.empty", true))));
		}
		
		// Update currencies rates
		if ($rate_processor != null) {
			// Set default currency
			$default_currency = $this->ifSet($company_settings['default_currency']);
			
			// Get the exchange rate padding value
			$pad_value = $this->ifSet($company_settings['exchange_rates_padding'], 0);
			
			$currencies = $this->getAll($company_id);
			foreach ($currencies as $currency) {
				// Default currency rate must be 1
				if ($currency->code == $default_currency) {
					$this->edit($currency->code, $company_id, array('exchange_rate'=>1, 'exchange_updated'=>date("c")));
					continue;
				}
				...
				if ($rate) {
					...
					$this->edit($currency->code, $company_id, array('exchange_rate'=>$rate['rate'], 'exchange_updated'=>date("c")));
				}
			}
		}
	}
