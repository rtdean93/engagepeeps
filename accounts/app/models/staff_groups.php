...
class StaffGroups extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("staff_groups"));
	}
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			// Add group notices
			foreach ($this->ifSet($vars['notices'], array()) as $notice) {
				$this->addNotice(array('staff_group_id'=>$staff_group_id, 'action'=>$notice));
			}
	...
	public function edit($staff_group_id, array $vars) {
		...
		$rules = $this->getRules();
		$rules['staff_group_id'] = array(
			'exists' => array(
				'rule' => array(array($this, "validateExists"), "id", "staff_groups"),
		...
		if ($this->Input->validates($vars)) {
			...
			Cache::clearCache("nav_staff_group_" . $staff_group_id, $vars['company_id'] . DS . "nav" . DS);
		}		
	}
	...
	public function delete($staff_group_id) {
		...
		$vars = array('num_staff'=>$num_staff);
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->from("staff_groups")->
				...
				delete(array("staff_groups.*", "staff_group.*"));
			...
			// Clear nav cache for this group
			Cache::clearCache("nav_staff_group_" . $staff_group_id, $staff_group->company_id . DS . "nav" . DS);
	...
	public function addNotice(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->duplicate("action", "=", $vars['action'])->
				insert("staff_group_notices", $vars, array("staff_group_id", "action"));
		}
	}
	...
	public function getUsersGroups($user_id) {
		$fields = array("staff.id"=>"staff_id", "staff_groups.id"=>"group_id", "staff_groups.company_id");
	...
	public function getStaffGroupByStaff($staff_id, $company_id) {
		$staff_group = $this->Record->select(array("staff_groups.*"))->from("staff_group")->
			innerJoin("staff_groups", "staff_groups.id", "=", "staff_group.staff_group_id", false)->
	 ...
	 */
	public function getList($company_id, $page=1, $order_by=array('name'=>"ASC")) {
	...
	public function getNotices($staff_group_id) {
		$fields = array("staff_group_notices.*", "email_groups.notice_type");
	 ...
	 *
	 * @param string $aco The ACO to set "allow" access permissions in the ACL
	...
	private function getNoticeRules(array $vars) {
		$rules = array(
			...
			'action' => array(
				'exists' => array(
					'rule' => array(array($this, "validateExists"), "action", "email_groups"),
					'message' => $this->_("StaffGroups.!error.action.exists", $this->ifSet($vars['action']))
				)
			)
		);
