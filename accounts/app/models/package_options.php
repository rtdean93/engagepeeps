...
class PackageOptions extends AppModel {
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$fields = array("company_id", "label", "name", "type");
	...
	public function edit($option_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			// Add the package option
			$fields = array("label", "name", "type");
			$this->Record->where("id", "=", $option_id)->update("package_options", $vars, $fields);
			
			// Delete existing package option values and pricing
			$this->deleteOptionValues($option_id);
	...
	public function deleteOptionValues($option_id) {
		$this->Record->from("package_option_values")->
			...
			delete(array("package_option_values.*", "package_option_pricing.*", "pricings.*"));
	}
	...
	/**
	 * Adds package option values (and subsequent pricing) for a package option
	 *
	...
	/**
	 * Adds package option pricing for a specific package option value
	 *
	...
	private function addOptionGroups($option_id, array $option_groups) {
		...
		foreach ($option_groups as $option_group) {
			...
			$option_order = $this->Record->select(array("order"))->from("package_option_group")->
				where("option_id", "=", $option_id)->
				where("option_group_id", "=", $option_group)->fetch();
			$order = 0;
			if ($option_order)
	...
	/**
	 * Fetching pricing for the given option with the given value
	 *
	 ...
	 * @param string $convert_currency The currency to convert to
	 ...
	 */
	public function getFields($package_id, $term, $period, $currency, $vars = null, $convert_currency = null) {
		...
		foreach ($options as $option) {
			...
			switch ($option->type) {
				...
				case "quantity":
					...
					#
					# TODO: Set javascript in HTML to enable javascript slider?
					#
					unset($value);
					break;
			}
