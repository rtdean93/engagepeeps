 ...
 *
 * Requires PHP 5.1 or greater. For enhanced security, PHP 5.1.2 or greater is
 * recommended.
 *
 * @license http://opensource.org/licenses/mit-license.php MIT License
 * @copyright Copyright (c) 2012, Phillips Data Inc.
 */
final class LicenseManager {
	 ...
	 * @param string $install_path The path to the installation of which to validate (simply sent to the validation server)
	 ...
	 */
	public function __construct($path_to_phpseclib, $signature_mode="md5", array $signatures=null, $install_path=null) {
	 ...
	 *
	 * @param array $signatures An array of key/value pairs where each key is the file path and each value is the signature,
	 * or an array of possible signatures, to verify against
	 * @return boolean True if the signatures verify, false otherwise
	 */
	 ...
	 * @return array An array of computed signatures in key/value pairs where each key is the file path and each value is the signature
	 */
	 ...
	 * 		- invalid_location The license is invalid for this domain, IP, or directory path. Request reissue, then try LicenseManager::requestData()
	...
	public function validate($license_data, $ttl) {
		...
		if ($license_data && $this->verifySignature($license_data, $this->keys, true)) {
			...
			$data = unserialize($this->decryptData($license_data, $this->keys));
			...
			elseif (isset($data['domain']) && !in_array($server_info['domain'], (array)$data['domain']) && !in_array("*", (array)$data['domain']))
				$status = "invalid_location";
			// Verify license ip (if given)
			elseif (isset($data['ip']) && !in_array($server_info['ip'], (array)$data['ip']) && !in_array("*", (array)$data['ip']))
				$status = "invalid_location";
			// Verify license path (if given)
			elseif (isset($data['path']) && !in_array($server_info['path'], (array)$data['path']) && !in_array("*", (array)$data['path']))
				$status = "invalid_location";			
			...
			return array(
				...
				'allow_reissue' => isset($data['allow_reissue']) ? $data['allow_reissue'] : null,
	 ...
	 * @param int $timeout The number of seconds to wait for a response from the license server before closing the connection
	...
	public function requestData(array $custom_data = null, $timeout = 10) {
		...
		$data = $this->encrypt(serialize($this->getServerInfo() + (array)$custom_data), $this->keys->aes_key, $this->keys->iv);

		$params = array(
			'key' => $this->keys->license_key,
			'data' => $this->formatResponse($data, $this->signRsa($data, $this->keys->public_key, $this->keys->hmac_key))
		);
	 ...
	 * @param boolean $encrypted True if the response data is encrypted, false otherwise
	 ...
	 * 	- public_key
	 * 	- hmac_key
	 * @param boolean $encrypted True if the data is encrypted, false otherwise
	 * @return boolean True if the data can be trusted, false otherwise
	 */
	 ...
	 *
	 * @param string $data The data to be verified. A hash of this data will automatically be computed and the hash verified.
	...
	private function loadLib($package, $lib) {
		...
		$file_name = $this->path_to_phpseclib . DIRECTORY_SEPARATOR . $package . DIRECTORY_SEPARATOR . $lib . ".php";
		
		// Verify the signature of the request library
		if (isset($this->signatures[$file_name])) {
			// Verify the signature of the given value matches that which is set
			if ($this->hash($file_name, $this->signature_mode) != $this->signatures[$file_name]) {
				return;
			}
		}
