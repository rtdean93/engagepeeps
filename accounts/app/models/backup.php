...
class Backup extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("backup"));
	}
	...
	public function download() {
		...
		if (!isset($this->Download))
			Loader::loadComponents($this, array("Download"));
	...
	public function sendBackup($type="all") {
		...
		Loader::loadComponents($this, array("SettingsCollection", "Security", "Net"));
		...
		if ((($type=="all" || $type=="ftp") && isset($settings['ftp_host']) && !empty($settings['ftp_host'])) ||
			(($type=="all" || $type=="amazons3") && isset($settings['amazons3_access_key']) && !empty($settings['amazons3_access_key']))) {
			$file = $this->buildDump();
		}
		...
		// SFTP the file
		if (($type=="all" || $type=="ftp") && isset($settings['ftp_host']) && !empty($settings['ftp_host'])) {
			$this->Net_SFTP = $this->Security->create("Net", "SFTP", array($settings['ftp_host'], $settings['ftp_port']));
			$uploaded = false;
			...
			if ($this->Net_SFTP->login($settings['ftp_username'], $settings['ftp_password']) &&
				...
				// Upload the file
				if ($this->Net_SFTP->put(basename($file), $file, NET_SFTP_LOCAL_FILE))
					$uploaded = true;
			}
			
			if (!$uploaded)
				$errors = array('ftp_failed'=>$this->_("Backup.!error.ftp_failed", true));
		}
		
		// Upload the file to Amazon S3
		if (($type=="all" || $type=="amazons3") && isset($settings['amazons3_access_key']) && !empty($settings['amazons3_access_key'])) {
			$this->AmazonS3 = $this->Net->create("AmazonS3", array($settings['amazons3_access_key'], $settings['amazons3_secret_key']));
			...
			if (!$uploaded)
				$errors = array_merge($errors, array('amazons3_failed'=>$this->_("Backup.!error.amazons3_failed", true)));
		}
		
		if (!empty($errors))
			$this->Input->setErrors(array($errors));
	 ...
	 *
	 * @return array An array of key/value pairs where each key is a number of hours and each value is a string representing that number of hours.
	 */
	public function frequencies() {
		$frequency = array('never' => Language::_("Backup.frequencies.never", true));
		...
		for ($i=24, $j=12; $i<=168; $i+=$j) {
			if ($i > 36)
				$j = 24;
			$frequency[$i] = Language::_(($i == 24 ? "Backup.frequencies.day" : "Backup.frequencies.days"), true, round($i/24, 2));
		}
		return $frequency;
	}
