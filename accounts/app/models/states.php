...
class States extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("states"));
	}
	 ...
	 * @param string $sort_by The field to sort on
	 * @param array $order_by A set of key/value fields to order by where each key is the field and each value is the direction (asc/desc)
	 * @return array An array of stdClass state objects
	 */
	public function getList($country=null, array $order_by = array("name" => "asc")) {
		$this->Record->select(array("country_alpha2","code","name"))->from("states");
			
		if ($country !== null)
			$this->Record->where("country_alpha2", "=", $country);
		
		$order_by = array_merge(array('country_alpha2' => "asc"), $order_by);
	...
	public function get($country, $code) {
		return $this->Record->select(array("country_alpha2","code","name"))->
	...
	public function edit($country, $code, array $vars) {
		...
		// Remove country_alpha2 in_use constraint
	...
	private function getRules(array $vars) {
		$rules = array(
			...
			'country_alpha2' => array(
				...
				'in_use' => array(
					'rule' => array(array($this, "get"), $this->ifSet($vars['code'])),
					'negate' => true,
					'message' => $this->_("States.!error.country_alpha2.in_use", $this->ifSet($vars['country_alpha2']), $this->ifSet($vars['code']))
				)
			),
			'name' => array(
				'format' => array(
