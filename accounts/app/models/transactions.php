...
class Transactions extends AppModel {
	...
	private function searchTransactions($query) {
		...
		$this->Record = $this->Record->select()->appendValues($values)->from(array($sub_query_sql => "temp"))->
			...
			orLike("temp.transaction_id", "%" . $query . "%");
		...
		if ($this->verifyApply($vars)) {
			...
			for ($i=0, $num_amounts=count($vars['amounts']); $i<$num_amounts; $i++) {
				...
				if ($vars['amounts'][$i]['amount'] > 0)
					$this->Record->duplicate("amount", "=", "amount + '" . ((float)$vars['amounts'][$i]['amount']) . "'", false)->insert("transaction_applied", $vars['amounts'][$i], $fields);
	...
	public function getTypes() {
		$fields = array("id", "name", "is_lang");
		
		$transaction_types = $this->Record->select($fields)->from("transaction_types")->fetchAll();
	...
	public function getType($type_id) {
		...
		$transaction_type = $this->Record->select($fields)->from("transaction_types")->where("id", "=", $type_id)->fetch();
