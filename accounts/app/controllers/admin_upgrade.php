...
class AdminUpgrade extends AppController {
	...
	public function preAction() {
		...
		$this->view->setView(null, "default");
		$this->structure->setDefaultView(APPDIR);
		$this->structure->setView(null, "default");
		
		$this->components(array("Upgrades"));
		$this->uses(array("Settings"));
		...
		if ($data)
			$this->database_version = $data->value;
		else {
	...
	private function processCli() {
		...
		$this->Upgrades->start($this->database_version, $this->file_version, array($this->Console, "progressBar"));
