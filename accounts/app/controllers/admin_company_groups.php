...
class AdminCompanyGroups extends AppController {
	...
	public function add() {
		...
		if (!empty($this->post)) {
			...
			// Set invoice checkbox setting values if not given
			...
			if (($errors = $this->ClientGroups->errors())) {
				...
				if (!isset($this->post['use_company_settings']) || $this->post['use_company_settings'] != "true") {
					// Remove client group fields from being added as client group settings
					unset($this->post['company_id'], $this->post['name'], $this->post['color'], $this->post['description']);
					$this->ClientGroups->setSettings($client_group_id, $this->post);
				}
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->ClientGroups->errors())) {
				...
				if (!isset($this->post['use_company_settings']) || $this->post['use_company_settings'] != "true") {
					// Remove client group fields from being added as client group settings
					unset($this->post['company_id'], $this->post['name'], $this->post['color'], $this->post['description']);
					$this->ClientGroups->setSettings($group->id, $this->post);
				}
				else {
