...
class AdminCompanyCurrencies extends AppController {
	...
	public function preAction() {
		...
		$this->uses(array("Companies", "Currencies", "Navigation"));
	...
	public function setup() {
		...
		$currencies = $this->Form->collapseObjectArray($currencies, array("code"), "code", " ");
		// Get the exchange rate processors
		$processors = array(""=>Language::_("AppController.select.please", true));
		
		$this->set("exchange_rate_processors", array_merge($processors, $this->Currencies->getExchangeRateProcessors()));
	...
	public function active() {
		...
		$currencies = $this->Currencies->getList($this->company_id, $page, array($sort=>$order));
	...
	public function add() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->Currencies->errors())) {
				...
				// Success
				$this->flashMessage("message", Language::_("AdminCompanyCurrencies.!success.add_created", true, $this->post['code']));
		...
		$exchange_rates_auto_update = $this->SettingsCollection->fetchSetting($this->Companies, $this->company_id, "exchange_rates_auto_update");
	...
	public function edit() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->Currencies->errors())) {
				...
				// Success
				$this->flashMessage("message", Language::_("AdminCompanyCurrencies.!success.edit_updated", true, $currency_code));
				$this->redirect($this->base_uri . "settings/company/currencies/active/");
			}
		}
	...
	public function delete() {
		...
		if (($errors = $this->Currencies->errors())) {
			...
			// Success
			$this->flashMessage("message", Language::_("AdminCompanyCurrencies.!success.delete_deleted", true, $currency->code));
		}
