	...
	private function formatOptionValueData(array $vars) {
		...
		if (isset($vars['values']) && is_array($vars['values'])) {
			...
			foreach ($vars['values'] as &$value) {
				...
				// Remove value to use default value when created iff value is not set or this is a quantity type where a value does not apply
				if (!isset($value['value']) || $value['value'] == "" || (isset($vars['type']) && $vars['type'] == "quantity"))
					unset($value['value']);
				
				// Remove the min/max/step fields if this is not a quantity type
				if (!isset($vars['type']) || $vars['type'] != "quantity")
					unset($value['min'], $value['max'], $value['step']);
				
				// Remove max to use default value
