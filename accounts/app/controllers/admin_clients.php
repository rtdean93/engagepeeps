...
class AdminClients extends AppController {
	...
	public function view($content=null) {
		...
		$this->set("plugin_actions", $this->PluginManager->getActions($this->company_id, "action_staff_client"));
	...
	public function edit() {
		...
		if (!empty($this->post)) {
			...
			$this->Clients->setSettings($client->id, $settings);
		...
		$this->set("countries", $this->Form->collapseObjectArray($this->Countries->getList(), array("name", "alt_name"), "alpha2", " - "));
		$this->set("states", $this->Form->collapseObjectArray($this->States->getList($vars->country), "name", "code"));
		$this->set("currencies", $this->Form->collapseObjectArray($this->Currencies->getList($this->company_id), "code", "code"));
		$this->set("languages", $this->Form->collapseObjectArray($this->Languages->getAll($this->company_id), "name", "code"));
	...
	public function makePayment() {
		...
		if (isset($this->get[1]) && !(($invoice = $this->Invoices->get($this->get[1])) && $invoice->status == "active" && $invoice->client_id == $client->id && $invoice->date_closed == null))
	...
	public function recordPayment() {
		...
		// If an invoice ID is set, fetch the invoice and ensure it is active and belongs to this client
		if (isset($this->get[1]) && !(($invoice = $this->Invoices->get($this->get[1])) && $invoice->status == "active" && $invoice->client_id == $client->id))
			$this->redirect($this->base_uri . "clients/" . $this->get[0] . "/");
			...
			case "1":
				...
				if (!empty($this->post) && count($this->post) > 2) {
					...
					if (isset($vars->invoice_id)) {
						...
						$this->Transactions->verifyApply($apply_amounts, false, $vars->amount);
					}
	...
	public function deleteDraftInvoice() {
		...
		if (($errors = $this->Invoices->errors())) {
			...
			$this->flashMessage("message", Language::_("AdminClients.!success.deletedraftinvoice_deleted"));
		}
	...
	public function addService() {
		$this->uses(array("Services", "Packages", "PackageGroups", "ModuleManager", "Coupons"));
		$step = "basic";
	...
	public function editService() {
		...
		// Suspend/Unsuspend/Cancel/Change Package/Module Options
			...
			if (!empty($this->post) && isset($this->post['section']) && $service->status != "canceled") {
				...
				switch ($this->post['section']) {
						...
						switch ($this->post['action']) {
							...
							case "change_renew":
								...
								if ($allow_renew_date) {
									...
									$this->Services->edit($service->id, $this->post, true);
								}
								break;
						}
						break;
					...
					case "package":
					case "module":
						$this->Services->edit($service->id, $this->post);
						break;
				}
		...
		if ($service->package_group_id)
			list($package_options, $package_attributes) = $this->listPackages($service->package_group_id, true);
		
		$module_name = $module->getName();
		$this->set("form", $this->partial("admin_clients_editservice_" . ($service->status == "pending" || $service->status == "in_review" ? "pending" : "basic"), compact("periods", "service", "package", "module_name", "fields", "html", "actions", "terms", "vars", "package_options", "package_attributes", "module_row_ids", "module_row_name")));
	...
	private function createService($details, $item) {
		$this->uses(array("Clients", "Coupons", "Invoices", "Services"));
		...
		if (isset($details['coupon'])) {
			$coupon = $this->Coupons->getForPackages($details['coupon'], null, $package_ids);
	...
	private function renderServiceStep($step, $package, $package_group, $client) {
		...
		switch ($step) {
			...
			case "confirm":
				...
				$item->package_group = $package_group;
				$item->package_options = $this->PackageOptions->getByPackageId($package->id);
				if (isset($item->configoptions)) {
					foreach ($item->package_options as $option) {
						if (array_key_exists($option->id, $item->configoptions)) {
							...
							$option->term = $this->getOptionTerm($pricing, $option->id, $item->configoptions[$option->id]);
						}
					}
					unset($option);
				}

				// Packages ordered (for pricing calculations)
				$packages = array(
					array(
				...
				foreach ($addons as &$addon) {
					...
					$addon->package = $this->Packages->getByPricingId($addon->pricing_id);
					$addon->package->terms = $this->getPackageTerms($addon->package);
					$addon->package_group = $this->PackageGroups->get($addon->package_group_id);
					$addon->package_options = $this->PackageOptions->getByPackageId($addon->package_id);
					if (isset($addon->configoptions)) {
						foreach ($addon->package_options as $option) {
							if (array_key_exists($option->id, $addon->configoptions)) {
								...
								$option->term = $this->getOptionTerm($pricing, $option->id, $addon->configoptions[$option->id]);
							}
						}
						unset($option);
					}
				...
				$line_totals = $this->Packages->calcLineTotals($client->id, $packages, $coupon);
				...
				$this->set("package", $package);
				$this->set("form", $this->partial("admin_clients_addservice_confirm", compact("item", "addons", "order_info", "addon_groups", "line_totals", "vars", "status", "invoice")));
				break;
	 ...
	 */
	private function getOptionTerm($package_pricing, $option_id, $value) {
		...
		if ($value) {
			$price = $this->PackageOptions->getValuePrice($value->id, $package_pricing->term, $package_pricing->period, $package_pricing->currency);
			...
			return Language::_($term, true, $price->term, $price->term != 1 ? $plural_periods[$price->period] : $singular_periods[$price->period], $this->CurrencyFormat->format($price->price, $price->currency));
		}
	 ...
	 */
	private function listPackages($parent_group_id = null, $return = false) {
		...
		foreach ($package_groups as $package_group) {
			...
			foreach ($temp_packages as $package) {
				...
				$packages[$package->status][] = array('name' => $package->name, 'value' => $package_group->id . "_" . $package->id);
			}
		}
		...
		foreach ($packages as $status => $packs) {
			...
			$package_attributes[$status] = array('disabled' => "disabled");
