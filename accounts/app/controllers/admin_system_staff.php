...
class AdminSystemStaff extends AppController {
	...
	public function preAction() {
		...
		$this->uses(array("Navigation", "Settings", "Staff", "StaffGroups"));
	...
	public function manage() {
		...
		$this->set("staff", $this->Staff->getList(null, $status, $page, array($sort => $order)));
	...
	public function status() {
		// Redirect if staff ID not given or status is invalid
		if (empty($this->post['id']) || !($staff = $this->Staff->get((int)$this->post['id'])) ||
			empty($this->post['status']) || !$this->Staff->validateStatus($this->post['status']))
			$this->redirect($this->base_uri . "settings/system/staff/manage/");
		
		// Update the status of this staff member
		$vars = array('status' => $this->post['status']);
		$this->Staff->edit($staff->id, $vars);
		
		$this->flashMessage("message", Language::_("AdminSystemStaff.!success.staff_updated", true));
		$this->redirect($this->base_uri . "settings/system/staff/manage/" . ($this->post['status'] == "inactive" ? "active" : "inactive"));
	}
	...
	public function groups() {
		...
		// Get staff groups
		$staff_groups = $this->StaffGroups->getList(null, $page, array($sort => $order));
	...
	public function addGroup() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->StaffGroups->errors())) {
				...
				// Success
				$this->flashMessage("message", Language::_("AdminSystemStaff.!success.group_added", true, $this->post['name']));
	...
	public function editGroup() {
		...
		if (!$vars) {
			...
			$permissions = $this->Permissions->fromAcl("staff_group_" . $staff_group->id, "staff", $staff_group->company_id);
			...
			// Set email notices
			$notices = (!empty($vars->notices) ? $vars->notices : array());
			$vars->notices = array();
			foreach ($notices as $notice)
				$vars->notices[] = $notice->action;
		}
		
		// Determine if this staff group is assigned to the current staff member - need to caution them about making changes
		$users_staff_group = $this->StaffGroups->getStaffGroupByStaff($this->Session->read("blesta_staff_id"), $this->company_id);
	...
	public function deleteGroup() {
		...
		if (($errors = $this->StaffGroups->errors())) {
			...
			$this->flashMessage("message", Language::_("AdminSystemStaff.!success.group_deleted", true, $this->Html->_($staff_group->name, true)));
		}
		$this->redirect($this->base_uri . "settings/system/staff/groups/");
	}
	...
	private function getEmailGroups($type) {
		...
		$email_groups = array_merge($this->EmailGroups->getAllByNoticeType($type), $this->EmailGroups->getAllByNoticeType($type, null, false));
