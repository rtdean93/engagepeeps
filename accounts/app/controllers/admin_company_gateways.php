...
class AdminCompanyGateways extends AppController {
	...
	public function preAction() {
		...
		$this->uses(array("GatewayManager", "Navigation"));
	...
	public function installed() {		
		...
		$this->set("gateway_types", array("merchant", "nonmerchant"));
	...
	public function manage() {
		...
		if (!isset($this->get[0]) || !($gateway_info = $this->GatewayManager->get((int)$this->get[0]))
			|| ($gateway_info->company_id != $this->company_id))
			$this->redirect($this->base_uri . "settings/company/gateways/installed/");
		
		$this->uses(array("Currencies"));
		$this->components(array("Gateways"));
		...
		if (!empty($this->post)) {
			...
			$currencies = isset($this->post['currencies']) ? $this->post['currencies'] : array();
			
			// Remove currencies from meta data
			unset($this->post['currencies']);
			
			$this->GatewayManager->edit($this->get[0], array('meta'=>$this->post, 'currencies'=>$currencies));
		...
		foreach ($gateway->getCurrencies() as $currency) {
			...
			if ($gateway_info->type == "merchant" && $this->GatewayManager->verifyCurrency($currency, $this->company_id, $gateway_info->id))
		...
		$this->set("vars", $vars);
		$this->structure->set("page_title", Language::_("AdminCompanyGateways.manage.page_title", true, $gateway_info->name));
	}
	...
	public function install() {
		...
		$gateway_id = $this->GatewayManager->add(array('class'=>$this->post['id'],'company_id'=>$this->company_id,'type'=>isset($this->post['type']) ? $this->post['type'] : "merchant"));
